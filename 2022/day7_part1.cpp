...
class File {
   public:
    ...
    void updateSize(std::uint64_t new_size) { size = new_size; }
...
class fNode {
   public:
    ...
    fNode(std::string name, std::shared_ptr<fNode> parent)
        : parent{parent}, subdirectories{}, files{}, name{name}, size{0} {}

    void addFile(std::string name, bool isFile) { files[name] = std::make_shared<File>(name, this); }

    void addSubdirectory(std::string name, bool isFile) { subdirectories[name] = std::make_shared<fNode>(name, this); }
...
class FileSystem {
   public:
    ...
    void cd(std::string name) { current = root->subdirectories[name]; }
